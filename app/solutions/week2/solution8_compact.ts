...
function isTreeVisibleFromBorders(trees: number[][], x: number, y: number): boolean {
...
function getNumberOfTreesVisibleFromHeight(trees: number[], height: number): number {
    const lastNotVisibleTree = trees.findIndex(v => v >= height);
    return lastNotVisibleTree === -1 ? trees.length : lastNotVisibleTree + 1;
}

function replaceTreeScoreIfHigher(trees: number[][], x: number, y: number, highScore: {score: number}): void {
...
export function printSolutions8() {
    const input = fs.readFileSync('./app/res/week2/input8.txt').toString();
